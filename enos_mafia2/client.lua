...
Citizen.CreateThread(function()
    ...
    local mafia2map = AddBlipForCoord(334.75, -2039.65, 21.1)

    SetBlipSprite(mafia2map, 310)
    SetBlipColour(mafia2map, 5)
    SetBlipScale(mafia2map, 0.80)
    SetBlipAsShortRange(mafia2map, true)
    BeginTextCommandSetBlipName('STRING')
    AddTextComponentString("Quartier")
    EndTextCommandSetBlipName(mafia2map)
end)
...
Citizen.CreateThread(function()
    ...
    while true do
        ...
        for k,v in pairs(Config.pos) do
            if ESX.PlayerData.job and ESX.PlayerData.job.name == 'mafia2' or ESX.PlayerData.job2 and ESX.PlayerData.job2.name == 'mafia2' then 
...
RMenu.Add('garagemafia2', 'main', RageUI.CreateMenu("Garage", "Garage mafia2"))

Citizen.CreateThread(function()
    while true do
        RageUI.IsVisible(RMenu:Get('garagemafia2', 'main'), true, true, true, function() 
        ...
        while true do
            ...
            if ESX.PlayerData.job and ESX.PlayerData.job.name == 'mafia2' or ESX.PlayerData.job2 and ESX.PlayerData.job2.name == 'mafia2' then    
                    ...
                    if IsControlJustPressed(1,51) then           
                        RageUI.Visible(RMenu:Get('garagemafia2', 'main'), not RageUI.Visible(RMenu:Get('garagemafia2', 'main')))
                    end   
                end
            end 
         end
...
function spawnuniCar(car)
    ...
    local plaque = "mafia2"..math.random(1,9)
...
RMenu.Add('coffremafia2', 'main', RageUI.CreateMenu("Coffre", " "))
RMenu.Add('coffremafia2', 'mettre', RageUI.CreateMenu("Coffre", " "))
RMenu.Add('coffremafia2', 'enlever', RageUI.CreateMenu("Coffre", " "))

Citizen.CreateThread(function()
    while true do

        RageUI.IsVisible(RMenu:Get('coffremafia2', 'main'), true, true, true, function()


            RageUI.ButtonWithStyle("Prendre objet", "Pour prendre un objet.", {RightLabel = "→"},true, function(Hovered, Active, Selected)
                if (Selected) then   
                RageUI.CloseAll()
                OpenGetStocksmafia2Menu()
                end
                end)
                RageUI.ButtonWithStyle("Déposer objet", "Pour déposer un objet.", {RightLabel = "→"},true, function(Hovered, Active, Selected)
                if (Selected) then   
                RageUI.CloseAll()
                OpenPutStocksmafia2Menu()
                end
                end)
...
Citizen.CreateThread(function()
        while true do
            ...
            if ESX.PlayerData.job and ESX.PlayerData.job.name == 'mafia2' or ESX.PlayerData.job2 and ESX.PlayerData.job2.name == 'mafia2' then  
                    ...
                    if IsControlJustPressed(1,51) then
                        RageUI.Visible(RMenu:Get('coffremafia2', 'main'), not RageUI.Visible(RMenu:Get('coffremafia2', 'main')))
                    end   
                end
               end 
        end
...
function OpenGetStocksmafia2Menu()
    ESX.TriggerServerCallback('esx_mafia2job:prendreitem', function(items)
        local elements = {}
        ...
        ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'stocks_menu', {
            ...
            ESX.UI.Menu.Open('dialog', GetCurrentResourceName(), 'stocks_menu_get_item_count', {
                ...
                if not count then
                    ...
                    menu2.close()
                    menu.close()
                    TriggerServerEvent('esx_mafia2job:prendreitems', itemName, count)
...
function OpenPutStocksmafia2Menu()
    ESX.TriggerServerCallback('esx_mafia2job:inventairejoueur', function(inventory)
        local elements = {}
        ...
        ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'stocks_menu', {
            ...
            ESX.UI.Menu.Open('dialog', GetCurrentResourceName(), 'stocks_menu_put_item_count', {
                ...
                if not count then
                    ...
                    menu2.close()
                    menu.close()
                    TriggerServerEvent('esx_mafia2job:stockitem', itemName, count)
...
function OpenGetWeaponMenu()
	...
	ESX.TriggerServerCallback('esx_mafia2job:getArmoryWeapons', function(weapons)
		local elements = {}
		...
		ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'armory_get_weapon',
		{
			...
			ESX.TriggerServerCallback('esx_mafia2job:removeArmoryWeapon', function()
...
function OpenPutWeaponMenu()
	...
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'armory_put_weapon',
	{
		...
		ESX.TriggerServerCallback('esx_mafia2job:addArmoryWeapon', function()
...
RMenu.Add('mafia2f6', 'main', RageUI.CreateMenu("mafia2", "Intéraction"))
...
Citizen.CreateThread(function()
    while true do
        ...
        RageUI.IsVisible(RMenu:Get('mafia2f6', 'main'), true, true, true, function()
        ...
        RageUI.ButtonWithStyle("Menotter/démenotter", nil, {RightLabel = "→"}, true, function(Hovered, Active, Selected)
            if (Selected) then 
                ...
                local target_id = GetPlayerServerId(target)
                if closestPlayer ~= -1 and closestDistance <= 3.0 then
                TriggerServerEvent('esx_mafia2job:handcuff', GetPlayerServerId(closestPlayer))
            else
                ESX.ShowNotification('Peronne autour')
            end
            end
            ...
            RageUI.ButtonWithStyle("Escorter", nil, {RightLabel = "→"}, true, function(Hovered, Active, Selected)
                if (Selected) then
					...
					local target_id = GetPlayerServerId(target)
					if closestPlayer ~= -1 and closestDistance <= 3.0 then
                TriggerServerEvent('esx_mafia2job:drag', GetPlayerServerId(closestPlayer))
            else
                ESX.ShowNotification('Peronne autour')
            end
            end
            ...
            RageUI.ButtonWithStyle("Mettre dans un véhicule", nil, {RightLabel = "→"}, true, function(Hovered, Active, Selected)
                if (Selected) then
					...
					local target_id = GetPlayerServerId(target)
					if closestPlayer ~= -1 and closestDistance <= 3.0 then
                TriggerServerEvent('esx_mafia2job:putInVehicle', GetPlayerServerId(closestPlayer))
            else
                ESX.ShowNotification('Peronne autour')
            end
                end
            ...
            RageUI.ButtonWithStyle("Sortir du véhicule", nil, {RightLabel = "→"}, true, function(Hovered, Active, Selected)
                if (Selected) then
					...
					local target_id = GetPlayerServerId(target)
					if closestPlayer ~= -1 and closestDistance <= 3.0 then
                TriggerServerEvent('esx_mafia2job:OutVehicle', GetPlayerServerId(closestPlayer))
            else
                ESX.ShowNotification('Peronne autour')
            end
            end
...
Citizen.CreateThread(function()
        while true do
            ...
            if ESX.PlayerData.job and ESX.PlayerData.job.name == 'mafia2' then  

                    if IsControlJustPressed(0, 167) then
                        RageUI.Visible(RMenu:Get('mafia2f6', 'main'), not RageUI.Visible(RMenu:Get('mafia2f6', 'main')))
                end   
        end 
    end
end)
...
Citizen.CreateThread(function()
    while true do
        Citizen.Wait(0)
            ...
            if ESX.PlayerData.job2 and ESX.PlayerData.job2.name == 'mafia2' then 
...
RegisterNetEvent('esx_mafia2job:handcuff')
AddEventHandler('esx_mafia2job:handcuff', function()
...
RegisterNetEvent('esx_mafia2job:drag')
AddEventHandler('esx_mafia2job:drag', function(cop)
...
RegisterNetEvent('esx_mafia2job:putInVehicle')
AddEventHandler('esx_mafia2job:putInVehicle', function()
...
RegisterNetEvent('esx_mafia2job:OutVehicle')
AddEventHandler('esx_mafia2job:OutVehicle', function(t)
...
function OpenBodySearchMenu(player)
	ESX.TriggerServerCallback('esx_mafia2job:getOtherPlayerData', function(data)
		local elements = {}
		...
		ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'body_search', {
			...
			if data.current.value then
				TriggerServerEvent('esx_mafia2job:confiscatePlayerItem', GetPlayerServerId(player), data.current.itemType, data.current.value, data.current.amount)
